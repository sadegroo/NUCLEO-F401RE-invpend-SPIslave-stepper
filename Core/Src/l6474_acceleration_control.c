...
void Init_L6474_Acceleration_Control(L6474_Init_t *gInitParams, float t_sample) {
	...
	hAccelCtrl.min_speed = gInitParams->minimum_speed_step_s;
	  ...
	  hAccelCtrl.period = RCC_SYS_CLOCK_FREQ / ( 1024 * (uint32_t)hAccelCtrl.min_speed);
...
void Run_L6474_Acceleration_Control(int16_t acceleration_input) {
	...
	case 10: // normal operation
	  ...
	  } else if (acceleration_input < -((int16_t)hAccelCtrl.max_accel)) {
		  hAccelCtrl.acceleration = -((int16_t)hAccelCtrl.max_accel);
	  } else {
...
void StepClockHandler_L6474_Acceleration_Control(void) {
	...
	hAccelCtrl.period = RCC_SYS_CLOCK_FREQ / ( 1024 * (uint32_t)hAccelCtrl.speed);
...
void PostProcess_StepClockHandler_L6474_Acceleration_Control(void) {
	...
	if (ISRFlag && hAccelCtrl.state == 10 && L6474_Board_Pwm1GetCounter() <= PWM_COUNT_SAFETY_MARGIN) {
		...
		if (BSP_MotorControl_GetDirection(0) == FORWARD) {
			hAccelCtrl.velocity = (int16_t)hAccelCtrl.speed;
		} else {
			hAccelCtrl.velocity = -((int16_t)hAccelCtrl.speed);
		}
	}
